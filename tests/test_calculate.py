...
def test_intersect(monkeypatch, tmpdir):
    ...
    vector_fp, data_fp = intersect(grid, 'name', square, 'name', dirpath=tmpdir, compress=False, cpus=None)
...
def test_calculate_remaining(tmpdir):
    ...
    data_fp = calculate_remaining(outside, 'name', remain_result, dirpath=tmpdir, compress=False)
    data = json.load(open(data_fp))
...
def test_intersections_from_intersection_specify_md(tmpdir):
    ...
    fp1, _ = intersections_from_intersection(inter_res, inter_res_decompressed, dirpath=tmpdir)
    data = json_importer(fp1)
    result = [
        # Order from geojson file
        [0, 'grid cell 3', 3097248058.207057],
    ...
    fp1, _ = intersections_from_intersection(new_fp, inter_res_decompressed, dirpath=tmpdir)
    data = json_importer(fp1)
    result = [
        # Order from geojson file
        [0, 'grid cell 3', 3097248058.207057],
...
def test_intersections_from_intersection_find_metadata(tmpdir):
    ...
    shutil.copy(
        ...
        os.path.join(tmpdir, 'intersection_result.json')
    )
