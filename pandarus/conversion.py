...
def convert_to_vector(filepath, dirpath=None, band=1):
    """Convert raster file at ``filepath`` to a vector file. Returns filepath of created
    vector file.

    ``dirpath`` should be a writable directory. If ``dirpath`` is no specified, uses the
    ...
    if dirpath is None:
        ...
        assert os.path.isdir(dirpath) and os.access(
            dirpath, os.W_OK
...
def clean_raster(fp, new_fp=None, band=1, nodata=None):
    ...
    ``new_fp``: String, optional. Filepath of the raster to create. If not provided,
    the new raster will have the same name as the existing file, but will be created in
...
def round_raster(in_fp, out_fp=None, band=1, sig_digits=3):
    ...
    """
    if out_fp is None:
        out_fp = os.path.join(tempfile.mkdtemp(), os.path.basename(in_fp))
